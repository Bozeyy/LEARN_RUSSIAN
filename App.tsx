...
const App: React.FC = () => {
  ...
  // On mélange la sélection à chaque fois que la catégorie ou le range change
    ...
    return (
      <div className="min-h-screen flex flex-col p-6 items-center justify-center">
        <div className="max-w-md w-full text-center mb-12">
          ...
          <p className="text-black font-black uppercase tracking-widest text-lg">Choisissez votre défi</p>
        </div>
        <div className="w-full max-w-sm space-y-6">
          <button 
            onClick={() => { setActivity('flashcards'); setStep('select-category'); }}
            className="w-full py-6 bg-[#F4E04D] nb-border nb-shadow text-3xl font-black text-black nb-press transition-all flex items-center justify-center gap-4 group"
          >
          ...
          <button 
            ...
            className="w-full py-6 bg-[#4D96FF] nb-border nb-shadow text-3xl font-black text-black nb-press transition-all flex items-center justify-center gap-4 group"
          >
            <span className="text-5xl">🧠</span> 
            <span>QUIZ</span>
          </button>
  ...
  if (step === 'select-range') {
    ...
    return (
      <div className="min-h-screen flex flex-col p-6 items-center justify-center">
        ...
        <h2 className="text-3xl font-black text-black mb-10 text-center uppercase bg-white nb-border p-2 rotate-1">Quel pack ?</h2>
        <div className="w-full max-w-sm space-y-4 max-h-[60vh] overflow-y-auto px-2">
  ...
  return (
    <div className="min-h-screen flex flex-col p-6 max-w-lg mx-auto">
      <div className="flex items-center justify-between mb-8">
        <button 
          onClick={resetAll}
          className="p-3 bg-white nb-border nb-shadow-sm text-black nb-press"
        >
        ...
        <div className="flex-1 flex flex-col">
          {quizScore === null ? (
            ...
            <div className="text-center py-12 px-6 bg-white nb-border nb-shadow flex flex-col items-center my-auto rotate-1">
              ...
              <h2 className="text-4xl font-black text-black mb-2 uppercase italic tracking-tighter">BIEN JOUÉ !</h2>
              ...
              <div className="w-full space-y-4">
                ...
                <button 
                  onClick={resetAll}
                  className="w-full py-4 bg-white text-black nb-border nb-shadow font-black text-xl nb-press uppercase"
                >
                  MENU PRINCIPAL
                </button>
              </div>
